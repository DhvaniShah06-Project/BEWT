  ...
  return (
    <>
      ...
      <div className="relative min-h-screen bg-[#020617] text-slate-200 font-sans overflow-hidden selection:bg-indigo-500/30">
          ...
          <div className="max-w-6xl mx-auto">
            <div className="bg-slate-950/60 backdrop-blur-xl border border-white/10 px-6 py-3 rounded-2xl flex justify-between items-center shadow-2xl">
              <Link href="/" className="flex items-center gap-2 group">
                ...
                <span className="text-white font-black tracking-tighter text-xl">AGENT<span className="text-indigo-400">LAB</span></span>
              </Link>

              {/* Desktop Menu */}
              <div className="hidden md:flex items-center gap-2">
                {['Agents', 'Use Cases', 'Pricing'].map((item) => (
                  <Link key={item} href={`#${item.toLowerCase().replace(' ', '-')}`} className="px-4 py-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 transition-all text-sm font-medium">
                    {item}
                  </Link>
                ))}
                <Link href="/signup" className="ml-4 bg-white text-black px-5 py-2 rounded-xl text-xs font-black hover:bg-indigo-400 transition-colors">
                  SIGN UP
                </Link>
              </div>
        ...
        <div className={`fixed inset-0 z-[60] bg-slate-950/95 backdrop-blur-2xl flex flex-col items-center justify-center gap-8 transition-transform duration-500 ${isMobileMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
          <button className="absolute top-8 right-8 text-slate-400 hover:text-white" onClick={() => setIsMobileMenuOpen(false)}>
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12" strokeWidth="2" strokeLinecap="round"/></svg>
          </button>
          {['Home', 'Agents', 'Use Cases', 'Pricing'].map((item) => (
            <Link key={item} href="#" className="text-3xl font-black text-white tracking-tighter hover:text-indigo-400" onClick={() => setIsMobileMenuOpen(false)}>{item}</Link>
          ))}
          <Link href="/signup" className="mt-4 bg-indigo-600 text-white px-10 py-4 rounded-2xl font-bold" onClick={() => setIsMobileMenuOpen(false)}>Sign Up Now</Link>
        </div>
        ...
        <main className="relative z-10 pt-40 pb-20 px-6 max-w-6xl mx-auto">
          <div className="text-center">
            ...
            <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full mb-8 backdrop-blur-md hover:border-indigo-500/50 transition-colors cursor-default">
              <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Smart, Fast, Always Active.</span>
              <span className="bg-indigo-500 text-[10px] px-2 py-0.5 rounded text-white font-bold">Launch App</span>
            </div>

            {/* Headline */}
            <h1 className="text-5xl md:text-8xl font-black text-white tracking-tighter leading-[0.9] mb-8">
              Build, Deploy & <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-cyan-400 to-purple-400 animate-gradient">Talk to AI Agents.</span>
            </h1>

            {/* Subtitle */}
            <p className="text-slate-400 text-lg md:text-xl max-w-2xl mx-auto mb-12 leading-relaxed">
              Design AI assistants that research, plan, and execute tasks — all powered by your prompts. The future of autonomous work is here.
            </p>
            ...
            <div className="mt-24 relative">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 opacity-40">
                    {[1,2,3,4].map(i => (
                        <div key={i} className="h-32 bg-slate-800/20 border border-white/5 rounded-2xl backdrop-blur-sm"></div>
                    ))}
                </div>
            </div>
